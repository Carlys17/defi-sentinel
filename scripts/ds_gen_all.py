...
def gen_copy():
    ...
    prompt = (
        "Write concise English text for 4 slides of a hackathon demo video "
    ...
    if not m:
        print("no json in response:", txt[:200])
        return None
    copy = json.loads(m.group(0))
    with open(f"{OUT}/copy.json", "w") as f:
        json.dump(copy, f, indent=1)
...
def submit_t2i(model, prompt, size="1664*928"):
    ...
    payload = {"model": model, "input": {"prompt": prompt}, "parameters": {"size": size, "n": 1}}
    d, err = post(
...
def submit_t2v(prompt, size="1280*720"):
    ...
    payload = {"model": "wan2.7-t2v", "input": {"prompt": prompt}, "parameters": {"size": size}}
    d, err = post(
...
def poll_task(tid, kind, outname, timeout_s=600, interval=18):
    ...
    while time.time() - start < timeout_s:
        ...
        if st == "SUCCEEDED":
            ...
            if not url:
                print("  no url:", json.dumps(out)[:300])
                return None
            path = f"{OUT}/{outname}"
            ok = download(url, path)
        ...
        if st in ("FAILED", "CANCELED", "UNKNOWN"):
            print("  task failed:", json.dumps(d)[:300])
            return None
        time.sleep(interval)
...
P_INTRO_V = (
    "Cinematic slow camera push-in on a glowing translucent cyan digital "
...
P_OUTRO_V = (
    "Slow cinematic pull-back revealing a constellation of glowing teal "
...
P_PROBLEM = (
    "Dark dramatic fintech scene: a crumbling red candlestick chart "
...
if __name__ == "__main__":
    ...
    tasks = {}
    tid = submit_t2v(P_INTRO_V)
    tasks[tid and ("v", "intro_clip.mp4", tid)] = True
    tid = submit_t2v(P_OUTRO_V)
    tasks[tid and ("v", "outro_clip.mp4", tid)] = True
    tid = submit_t2i("wan2.7-image-pro", P_ARCH)
    tasks[tid and ("i", "arch.png", tid)] = True
    tid = submit_t2i("z-image-turbo", P_PROOF)
    tasks[tid and ("i", "proof.png", tid)] = True
    tid = submit_t2i("wan2.6-t2i", P_SAFETY)
    tasks[tid and ("i", "safety_wan26.png", tid)] = True
    ...
    def sync_gap(needed=62):
        ...
        if wait > 0:
            print(f"... rate-limit wait {int(wait)}s ...")
            time.sleep(wait)
        last = time.time()
